...
High-performance, sharded, append-only journal system using Memory-Mapped I/O (mmap) for HTTP audit logging. Optimized for workloads exceeding 150k events/sec.
...
### 2.2 Decoupled Segments
Each shard manages two independent lifecycles:
* **Data Segment:** Binary event stream.
* **Index Segment:** Fixed-size lookup table for O(1) event location.
* **Benefit:** Prevents index overflow when processing large payloads (e.g., multi-megabyte bodies).
...
* **Direct Buffer Release:** Manual unmapping via `sun.misc.Unsafe` (requires JVM flag `--add-opens java.base/java.nio=ALL-UNNAMED`).
